...
def mapping(args):
    ...
    for epoch in range(1):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # if i > 10:
            #     break
            fix, flabels = data['img0'].to(device), data['seg0'].to(device)
            move, mlabels = atlas_input(args.batch_size, atlas, mask_atlas, device)
            # fix, flabels = data['img1'].to(device), data['seg1'].to(device)
            # print(inputs.shape, labels.shape)
            # 
            mlabels = mlabels.unsqueeze(1)
            flabels = flabels.unsqueeze(1)
            
            paired_rois = PairedRegions(masks_mov=mlabels, masks_fix=flabels,  device=device)
            ddf = paired_rois.get_dense_correspondence(transform_type='ddf', max_iter=int(1000), lr=1e-3, w_ddf=100.0, verbose=True)
            warped = warp_by_ddf(move, ddf)

            imgs.extend([img[0,...] for img in warped.cpu().detach().numpy()])
            if not args.noseg:
           
                warped_seg = warp_by_ddf(mlabels, ddf)
...
def parse_args():
    ...
    parser.add_argument('--path', type=str, default='1-uclH-data_ratio0.8', help='Path to data', choices=['picai_zones_ratio0.7', '1-uclH-data_ratio0.8'])
