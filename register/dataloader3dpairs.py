...
class dataset_loaders(data.Dataset):
    def __init__(self, path, 
        ...
        self.t2w_filenames = list(set(self.t2w_filenames))
        self.zone_filenames = [i.replace('t2', 'prostate_zones') for i
                             in self.t2w_filenames]
    ...
    def __getitem__(self, idx):
        while True:
            idx_next = np.random.randint(0, len(self.t2w_filenames))
            if idx_next != idx:
                break
        dict_data = {}
        scan, seg = self.load_data(idx)
        scan_next, seg_next = self.load_data(idx_next)
        dict_data = {'img0': scan, 'seg0': seg,
                     'key0': self.t2w_filenames[idx].split('/')[-1],
                     'img1': scan_next, 'seg1': seg_next,
                     }
        return dict_data
