...
class dataset_loaders(data.Dataset):
    def __init__(self, path, 
                ...
                istest=False, transform=None, ifbin=True):
        self.path = path
        self.phase = phase
        ...
        self.zone_filenames = [i.replace('t2w', 'prostate_zones') for i
    ...
    # convert glob path to filenames
        ...
        tmp=load_volfile(self.t2w_filenames[indices], **load_params).transpose(2,0,1,3)
        tmp = self.norm255(tmp)
        tmp=np.concatenate([tmp]*3, axis=-1)
        vols=np.rot90(tmp, k=1, axes=(1,2))
    ...
    def __getitem__(self, idx):
        ...
        dict_data = {'img': scan, 'seg': seg,
                     'key': self.t2w_filenames[idx].split('/')[-1]}
        return dict_data
